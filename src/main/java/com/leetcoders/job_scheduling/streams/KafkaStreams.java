...
public class KafkaStreams {
    ...
    private final PostgresHandler databaseHandler;
    private final KafkaProducer<String, String> topicProducer;
    private final KafkaConsumer<String, String> topicConsumer;

    public KafkaStreams(String kafkaServers, PostgresHandler databaseHandler) throws ExecutionException,
            InterruptedException {
        this.databaseHandler = databaseHandler;
        initializeTopics(kafkaServers);
        topicConsumer = createKafkaConsumer(kafkaServers);
    ...
    private void initializeTopics(String kafkaServers) throws ExecutionException, InterruptedException {
        Properties adminProperties = new Properties();
        adminProperties.setProperty(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaServers);
        try (Admin admin = Admin.create(adminProperties)) {
            var results = admin.listTopics().names().get();
            for (var topicName: List.of(RECOMMENDATION_REQUEST_TOPIC, RECOMMENDATION_RESPONSE_TOPIC)) {
                if (!results.contains(topicName)) {
                    NewTopic topic = new NewTopic(topicName, 10, (short) 3);
                    admin.createTopics(List.of(topic)).wait();
                }
            }
        }
    }
